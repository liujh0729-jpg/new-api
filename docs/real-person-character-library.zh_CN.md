...
全部 `/v1` 接口使用 `Authorization: Bearer <用户 API Key>`。会话和角色归属于 API Key 对应的用户；H5 地址可以交给被核验人打开，但不得把 API Key 一并暴露。API Key 调用方仍必须让本人完成 H5，不能纯后台绕过真人核验。公开调用方式见 [公开 OpenAPI 契约](openapi/public.json)。
