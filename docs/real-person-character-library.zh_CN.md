...
火山控制面 AK/SK 与出片使用的豆包视频通道必须属于同一火山账号和项目。管理员启用真人功能时必须绑定一个启用中的 `DoubaoVideo` 通道；每个真人请求会固定校验该通道 ID。
...
控制台角色库拆分为“公共角色 / 我的虚拟形象 / 我的真人 / 任务历史”。“我的真人”展示授权期限、商业属性和火山状态，支持身份核验、手动同步、创建视频和撤回授权。管理员设置包含独立真人额度、Premium 提示和豆包视频通道绑定。
...
上线验收至少应覆盖：同账号同项目联调、真人 H5 成功/拒绝/过期、零/一/多 Active Image、跨用户直接 Asset ID、跨通道、撤权并发、授权到期、火山下线、组合真人与官方虚拟人像、清理重试，以及 SQLite/MySQL/PostgreSQL 三种迁移。
