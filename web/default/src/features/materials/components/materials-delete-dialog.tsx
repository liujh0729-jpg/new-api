...
export function MaterialsDeleteDialog() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const result = await deleteMaterial(currentRow.id)
      if (result.success) {
        toast.success(t(SUCCESS_MESSAGES.MATERIAL_DELETED))
        setOpen(null)
        triggerRefresh()
      }
    } catch {
      toast.error(t('Failed to delete material'))
    } finally {
      setIsDeleting(false)
    }
  }
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t('This will permanently delete material')}{' '}
