...
2. **是否覆盖 AIPDD 模型价格？** Default to **否**. **是** authorizes the complete price-reconciliation procedure below: fetch the live authenticated catalog, let New API's built-in sync update eligible Token Market duration prices, then replace the remaining pricing entries belonging to the previous and current AIPDD model sets while preserving non-AIPDD entries. Seedance pricing must be written as a `by_resolution` matrix. **Only Seedance** requires `pricingBasis=display`, platform settlement fields `displayAmountAwcoinPerSecond` / `displayVideoInputAwcoinPerSecond`, and New API sale fields `suggestedRetailAwcoinPerSecond` / `suggestedRetailVideoInputAwcoinPerSecond`; a missing required Seedance field aborts the complete plan. Seedance `task_pricing` must be built only from the suggested-retail fields (Excel 对比原生价 / MSRP), never from `displayAmount*`, legacy modality fields, or BYOK fields. Eligible `token_market_media` + `per_second` models such as MiniMax H3 and Agnes duration variants do **not** require `suggestedRetail*`: the built-in catalog sync owns their `task_pricing` / `billing_mode` and converts authenticated `displayAmount*` prices with `AWCoin × rmbPerAwcoin ÷ site USDExchangeRate`. The helper must validate and preserve those post-sync entries instead of rebuilding them. A non-Seedance `per_unit` capability is supported only when `chargeConfig.unit=second`; it is written as flat `task_pricing` with the same RMB-anchored conversion. Never use catalog `usdPerAwcoin`, `priceVariants`, or a local/legacy `ModelPrice` fallback for manually rebuilt prices. Also ensure display currency is RMB (`DisplayInCurrencyEnabled=true`, `general_setting.quota_display_type=CNY`). **否** preserves all current local AIPDD pricing options, including any legacy flat Seedance pricing, does not call the catalog sync endpoint, and requires the application to restore runtime capabilities read-only from the same-origin last-known-good snapshot. In either case keep `AIPDD_CATALOG_SYNC_INTERVAL_MINUTES=0` unless the user separately and explicitly accepts recurring **partial** synchronization. Never claim that an update migrated all pricing when overwrite was declined or when only built-in catalog sync ran.
...
- `updated_prices` counts only built-in changes made for eligible available `token_market_media` capabilities using `pricingModel=per_second`, `currency=awcoin`, and `pricingBasis=display`. Those entries are converted with `display AWCoin × rmbPerAwcoin ÷ USDExchangeRate` and written to `billing_setting.task_pricing` / `billing_setting.billing_mode`. `suggestedRetail*` is not required for this path. It does not certify that Seedance, LLM, per-call, or `per_unit/second` prices were rebuilt.
...
3. Export only the saved catalog JSON payload from the newest `a_ip_dd_catalog_snapshots` row. This is the actual GORM table name for the acronym-heavy model on the SQLite deployment. Discover the equivalent table name from the target database schema before querying if the deployment uses another database or an older schema. Do not export the channel key, `.env`, cookies, or any database-wide dump. After the successful built-in sync, re-fetch the five pricing options plus `USDExchangeRate` and store that response as `options-after-sync.json`; do not pass the pre-sync backup to the helper. The post-sync options are authoritative for eligible `token_market_media/per_second` entries and must include their built-in `task_pricing` and `billing_mode`. Store the catalog, post-sync option response, and previous-model list in protected temporary JSON files. The option export passed to the helper must include a positive `USDExchangeRate` (read-only; never modify it during reconciliation). The catalog `awcoinRate` must include a positive `rmbPerAwcoin`; missing either value is a hard abort before any manual option write.
...
5. Run the bundled offline helper from a trusted local environment:
   ...
   The helper removes only entries owned by the previous/current AIPDD model sets, except that it preserves eligible Token Market duration entries just written by built-in sync. It validates those preserved entries and rebuilds the remaining AIPDD prices with the RMB-anchored conversion `storedUSD = AWCoin × rmbPerAwcoin ÷ site USDExchangeRate` (catalog `usdPerAwcoin` is unused):
   ...
   - **Seedance only** as suggested-retail `task_pricing`, with `unit=second`, a `by_resolution` tier for every catalog resolution, a per-tier no-reference-video price, an automatic per-tier `same`/`custom` reference-video policy, `group_ratio_policy=none` on the `480p` tier, and `billing_mode=task_pricing`;
   - eligible `token_market_media/per_second` models by preserving and validating the post-sync display-price `task_pricing` / `billing_mode` without changing their values; a missing `suggestedRetail*` field is valid on this path, but a missing built-in option entry or catalog-resolution mismatch aborts before manual writes;
   ...
   **Only Seedance** uses the dual-price AIPDD contract: platform settlement via `displayAmount*` and New API sale via `suggestedRetail*`. Normalize each Seedance resolution key with `trim + lowercase`, reject empty keys, keys longer than 128 characters, duplicates after normalization, and a `targetResolution` that does not match its normalized map key. Require `pricingBasis=display`. For every Seedance resolution, require positive `displayAmountAwcoinPerSecond`, positive `displayVideoInputAwcoinPerSecond`, positive `suggestedRetailAwcoinPerSecond`, positive `suggestedRetailVideoInputAwcoinPerSecond`, and positive `defaultFramesPerSecond`. Require positive `defaultDurationSeconds` except that a model name containing the version tokens `Seedance 2.5` may use the exact `-1` auto-duration sentinel; match the tokens case-insensitively with whitespace, hyphen, underscore, or dot separators, and continue rejecting zero and every other negative value. The sentinel is validation-only. Convert only Seedance suggested-retail AWCoin rates with the RMB-anchored factor into `task_pricing` USD/second. Legacy modality fields, platform `displayAmount*` fields, and BYOK fields must never participate in Seedance sale-price selection. Emit `reference_video_policy=same` when both suggested-retail prices are equal, otherwise emit `custom` and its explicit price. Emit `group_ratio_policy=none` for `480p`; omit it for other resolutions unless a future explicit rule requires otherwise. Never aggregate a maximum across resolutions, alias `2k/1440p` or `4k/2160p`, or read `priceVariants`, `minimumAwcoin`, an existing `ModelPrice`, a previous flat `task_pricing` value, or another fallback. A missing or invalid required Seedance field aborts plan generation; the same field may be absent on Token Market models without aborting.
6. Inspect the helper summary, not the complete option bodies. Require every active current catalog model to appear exactly once in the per-call, task-pricing, or tiered-expression lists; unavailable Token Market duration models may instead appear in `builtin_sync_removed_models`. Require every `builtin_synced_task_pricing_models` item to also appear in `task_pricing_models`, and require `summary.price_conversion=rmb_anchored`. Require `task_pricing_contract` to state that only Seedance requires suggested retail, Token Market per-second models preserve built-in display-price options without requiring `suggestedRetail*`, `per_unit/second` uses flat pricing, and no legacy `ModelPrice` fallback is used. If a required Seedance field, positive manual price, model ID, resolution tier, supported duration unit, or required post-sync Token Market option entry is missing, make no manual option writes.
...
9. Re-fetch `GET /api/option/` and require all five pricing values to equal the generated plan, and require `DisplayInCurrencyEnabled=true`, `general_setting.quota_display_type=CNY`, and an unchanged positive `USDExchangeRate`. Reject any reconciled Seedance entry that contains root-level `no_reference_video_unit_price`, `reference_video_policy`, or `reference_video_unit_price`; require `unit=second`, a non-empty `by_resolution`, an exact normalized tier-key match with the authenticated catalog, and `group_ratio_policy=none` on every `480p` tier. For every reconciled `per_unit/second` entry, reject `by_resolution`, require a positive root `no_reference_video_unit_price`, `reference_video_policy=same`, `unit=second`, and absence from `ModelPrice`. Spot-check at least one Seedance tier against `suggestedRetail*`. Separately verify every active `token_market_media/per_second` entry still exactly matches the post-sync display-price option captured before helper writes and has the catalog's normalized resolution set; do not require or inspect `suggestedRetail*` for that check. Then call `GET /api/pricing` and verify every model in `TaskPricingRequiredModels` has `billing_mode=task_pricing` and a valid positive task-pricing object. Seedance and Token Market matrix models must expose the active resolution intersection; flat duration models must remain visible without inventing resolution options. This read-only validation must replace a paid model request.
...
- When price overwrite was confirmed, every `TaskPricingRequiredModels` entry has a valid positive task-pricing object and `billing_mode=task_pricing`; Seedance entries have a non-empty suggested-retail `by_resolution` matrix, active Token Market per-second entries retain the built-in display-price matrix without any `suggestedRetail*` requirement, `per_unit/second` entries use the flat shape and do not remain in `ModelPrice`, and display options are `DisplayInCurrencyEnabled=true` and `general_setting.quota_display_type=CNY`. Built-in sync satisfies only the Token Market subset, not the remaining reconciliation.
