...
- When AIPDD price overwrite was confirmed, report that Seedance was verified as `by_resolution` matrix pricing, every `per_unit/second` model was verified as flat per-second task pricing, display currency is RMB, `USDExchangeRate` was left unchanged, and a Seedance spot-check confirmed `storedUSD × USDExchangeRate == AWCoin × rmbPerAwcoin`; when it was declined, report that the existing pricing shape was preserved and may still be legacy pricing.
