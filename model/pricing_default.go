...
var defaultCatalogModels = []defaultCatalogModel{
	{
		...
		VendorName:    "AIPDD",
		Description:   "基于 AIPDD ComfyUI 工作流的 Flux 图生图能力，输入参考图片并可结合提示词生成新图片。通过图片生成接口创建异步任务，任务完成后返回图片结果。",
		Icon:          aipddLogoPath,
		Tags:          "AIPDD,ComfyUI,图生图,图片生成,异步任务",
		ChannelType:   constant.ChannelTypeAIPDD,
		EndpointTypes: []constant.EndpointType{constant.EndpointTypeImageGeneration},
	},
	{
		ModelName:     constant.AIPDDModelFluxGGUFT2I,
		VendorName:    "AIPDD",
		Description:   "基于 AIPDD ComfyUI 工作流的 Flux 文生图能力，适合根据文本提示词生成图片。通过图片生成接口创建异步任务，任务完成后返回图片结果。",
		Icon:          aipddLogoPath,
		Tags:          "AIPDD,ComfyUI,文生图,图片生成,异步任务",
		ChannelType:   constant.ChannelTypeAIPDD,
		EndpointTypes: []constant.EndpointType{constant.EndpointTypeImageGeneration},
	},
	{
